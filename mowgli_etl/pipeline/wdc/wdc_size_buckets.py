    ...
    def generalize(self, wdc_product_type: WdcProductType, wdc_product_dimensions: WdcProductDimensions):
        ...
        if generic_dimensions.name in self.averages:
        	...
        	for field in tracked_dimension.dimension.keys():
        		...
        		if not dimension:
        			continue
        		dimension.value = (dimension.value * tracked_dimension.count + getattr(generic_dimensions, field.name).value) / (tracked_dimension.count + 1)
        	tracked_dimension.count += 1

        else:
        	self.averages[generic_dimensions.name] = generic_dimensions

        self.averages[generic_dimensions.name].bucket = self.__bucket(self.averages[generic_dimensions.name])
