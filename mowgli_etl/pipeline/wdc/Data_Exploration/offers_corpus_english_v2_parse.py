...
if not directory:
    print(
        'Sorry, there don\'t seem to be any clean files. Please make sure to run offers_corpus_english_v2_clean.py, or have a file ending in "clean.jsonl"'
    )
    exit(1)
	...
	*
    ...
    if description != None:
        dimensions = re.findall(
            "\d+(?: \d+)?\s?\w*\sx\s\d+(?: \d+)?\s?(?:x\s\d+\s?)?\w*", description
        )

        if len(dimensions) == 0:
            dimensions = re.findall(
                "\d+\s?\w+\s\d+\s?\w+\slead\sx\s\d+\s?\w+", description
            )
    ...
    if len(dimensions) == 0:
        if additional_info != None:
            dimensions = re.findall(
                "\d+(?: \d+)?\s?\w*\sx\s\d+(?: \d+)?\s?(?:x\s\d+\s?)?\w*",
                additional_info,
            )

            if len(dimensions) == 0:
                dimensions = re.findall(
                    "\d+\s?\w+\s\d+\s?\w+\slead\sx\s\d+\s?\w+", additional_info
                )
