...
def test_usf_extractor(pipeline_storage, strengths, url, usfclient):
    extractor = USFExtractor(http_client=usfclient)
    extraction = extractor.extract(force=False, storage=pipeline_storage, url=url, target="usf_test_data-master/usf_test_data.xml")
