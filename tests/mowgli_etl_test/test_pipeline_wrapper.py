...
SUBJECT_NODE = KgNode(id="testid", label="test label", pos="n", datasource=DATASOURCE)
EXACT_DUPLICATE_SUBJECT_NODE = KgNode(id="testid", label="test label", pos="n", datasource=DATASOURCE)
INEXACT_DUPLICATE_SUBJECT_NODE = KgNode(id="testid", label="test label variation", pos="n", datasource=DATASOURCE)
OBJECT_NODE = KgNode(id="testobject", label="test object", pos="n", datasource=DATASOURCE)
EDGE = KgEdge(subject=SUBJECT_NODE.id, object=OBJECT_NODE.id, predicate=DATASOURCE, datasource=DATASOURCE)
...
def test_extraneous_node(pipeline_storage):
    try:
        run((SUBJECT_NODE, OBJECT_NODE,
             KgEdge(subject=SUBJECT_NODE.id, object="externalnode", predicate=DATASOURCE,
