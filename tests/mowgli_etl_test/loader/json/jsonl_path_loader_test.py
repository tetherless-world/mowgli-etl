...
def test_load_path(pipeline_storage: PipelineStorage):
    with JsonlPathLoader().open(pipeline_storage) as loader:
        loader.load_path(Path(datasource="test_datasource", id="test_path1", path=("x1", "y1", "z1")))
        loader.load_path(Path(datasource="test_datasource", id="test_path2", path=("x2", "y2", "z2")))
