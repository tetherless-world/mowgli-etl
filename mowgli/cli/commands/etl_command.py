...
class EtlCommand(_Command):
    ...
    def __add_general_etl_args(self, arg_parser):
        ...
        arg_parser.add_argument(
            "--skip-dupe-check",
            action="store_true",
            help="Skip checking for duplicate nodes/edges during transform"
        )
        arg_parser.add_argument(
    ...
    def __call__(self, args):
        ...
        dupe_check = not bool(getattr(args, "skip_dupe_check", False))

        extract_kwds = pipeline_wrapper.extract(force=force_extract)
        graph_generator = pipeline_wrapper.transform(
            force=force_transform, dupe_check=dupe_check, **extract_kwds
        )
